...
export interface MLPRegressorOptions extends MLPCommonOptions {
  beta1?: number;
  beta2?: number;
  epsilon?: number;
}

export class MLPRegressor implements RegressionModel {
  coefs_: Matrix[] = [];
  intercepts_: Vector[] = [];
  nIter_ = 0;
  ...
  constructor(options: MLPRegressorOptions = {}) {
  ...
  fit(X: Matrix, y: Vector, sampleWeight?: Vector): this {
    ...
    const yMatrix: Matrix = y.map((value) => [value]);
    ...
    for (let iter = 0; iter < this.maxIter; iter += 1) {
      ...
      const loss = this.regressionLoss(prediction, yMatrix);
      this.loss_ = loss;
      nIter = iter + 1;

      let delta: Matrix = new Array(nSamples);
      for (let i = 0; i < nSamples; i += 1) {
        delta[i] = [prediction[i][0] - yMatrix[i][0]];
      }
  ...
  private regressionLoss(prediction: Matrix, target: Matrix): number {
    const n = prediction.length;
    let total = 0;
    for (let i = 0; i < n; i += 1) {
      const error = prediction[i][0] - target[i][0];
      total += error * error;
    }
    let l2 = 0;
    ...
    return total / (2 * n) + (this.alpha * l2) / (2 * n);
  }
  ...
  private assertFitted(): void {
    if (!this.fitted || this.coefs_.length === 0 || this.intercepts_.length === 0 || this.nFeaturesIn_ === null) {
      throw new Error("MLPRegressor has not been fitted.");
    }
  }
}
