...
export class MultinomialNB implements ClassificationModel {
  classes_: Vector = [];
  ...
  constructor(options: MultinomialNBOptions = {}) {
  ...
  fit(X: Matrix, y: Vector): this {
    ...
    if (classes.length < 2) {
      throw new Error("MultinomialNB requires at least two classes.");
    }
    ...
    for (let i = 0; i < nSamples; i += 1) {
      ...
      for (let j = 0; j < nFeatures; j += 1) {
        featureCount[classIndex][j] += X[i][j];
      }
    }
    ...
    for (let c = 0; c < classes.length; c += 1) {
      let total = 0;
      for (let j = 0; j < nFeatures; j += 1) {
        total += featureCount[c][j];
      }
      const denominator = total + this.alpha * nFeatures;
      for (let j = 0; j < nFeatures; j += 1) {
        featureLogProb[c][j] = Math.log((featureCount[c][j] + this.alpha) / Math.max(denominator, 1e-12));
      }
    }
  ...
  private assertNonNegativeMatrix(X: Matrix): void {
    for (let i = 0; i < X.length; i += 1) {
      for (let j = 0; j < X[i].length; j += 1) {
        if (X[i][j] < 0) {
          throw new Error(`MultinomialNB expects non-negative features. Found ${X[i][j]} at [${i}, ${j}].`);
        }
      }
    }
  }
  ...
  private assertFitted(): void {
    if (
      ...
      throw new Error("MultinomialNB has not been fitted.");
    }
  }
}
