...
export class ElasticNet implements RegressionModel {
  coef_: Vector = [];
  intercept_ = 0;
  nIter_ = 0;
  ...
  private validateOptions(): void {
    ...
    if (!Number.isFinite(this.l1Ratio) || this.l1Ratio < 0 || this.l1Ratio > 1) {
      throw new Error(`l1Ratio must be finite and in [0, 1]. Got ${this.l1Ratio}.`);
    }
  ...
  private assertFitted(): void {
    if (!this.fitted) {
      throw new Error("ElasticNet has not been fitted.");
    }
  }
}
