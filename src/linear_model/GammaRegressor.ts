...
export class GammaRegressor implements RegressionModel {
  coef_: Vector = [];
  intercept_ = 0;
  nIter_ = 0;
  ...
  constructor(options: GammaRegressorOptions = {}) {
  ...
  fit(X: Matrix, y: Vector, sampleWeight?: Vector): this {
    ...
    for (let i = 0; i < y.length; i += 1) {
      if (y[i] <= 0) {
        throw new Error("GammaRegressor requires strictly positive targets.");
      }
    }
    ...
    for (let iter = 0; iter < this.maxIter; iter += 1) {
      ...
      for (let i = 0; i < nSamples; i += 1) {
        ...
        const grad = 1 - y[i] / Math.max(mu, 1e-12);
  ...
  private assertFitted(): void {
    if (!this.fitted) {
      throw new Error("GammaRegressor has not been fitted.");
    }
  }
}