...
export interface MultiTaskElasticNetOptions {
  alpha?: number;
  l1Ratio?: number;
...
export class MultiTaskElasticNet {
  ...
  private estimators_: ElasticNet[] = [];
  private nFeaturesIn_: number | null = null;
  private nOutputs_: number | null = null;
  private fitted = false;

  constructor(options: MultiTaskElasticNetOptions = {}) {
  ...
  fit(X: Matrix, Y: Matrix): this {
    ...
    this.estimators_ = new Array<ElasticNet>(this.nOutputs_);
    ...
    for (let out = 0; out < this.nOutputs_; out += 1) {
      const estimator = new ElasticNet({
        alpha: this.alpha,
  ...
  private assertFitted(): void {
    if (!this.fitted || this.nFeaturesIn_ === null || this.nOutputs_ === null) {
      throw new Error("MultiTaskElasticNet has not been fitted.");
    }
  }
}