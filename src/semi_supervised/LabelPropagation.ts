...
export class LabelPropagation implements ClassificationModel {
  classes_: Vector = [0, 1];
  ...
  fit(X: Matrix, y: Vector, sampleWeight?: Vector): this {
    ...
    for (let iter = 0; iter < this.maxIter; iter += 1) {
      iterations = iter + 1;
      const next = multiply(transition, F);
      for (let i = 0; i < next.length; i += 1) {
        if (state.labeledMask[i]) {
          next[i] = state.Y[i].slice();
        }
      }
  ...
  private assertFitted(): void {
    if (
      ...
      throw new Error("LabelPropagation has not been fitted.");
    }
  }
}
