...
test("new estimators expose stable getParams/setParams roundtrip", () => {
  const estimators = [
    ...
    new MiniBatchKMeans({ nClusters: 2, batchSize: 4, randomState: 7 }),
    new Birch({ nClusters: 2, threshold: 0.3 }),
    new OPTICS({ minSamples: 2, maxEps: 0.5 }),
    new MeanShift({ bandwidth: 0.5, maxIter: 20 }),
    new AffinityPropagation({ damping: 0.7, maxIter: 40 }),
    ...
    new IncrementalPCA({ nComponents: 2, batchSize: 4 }),
    new FactorAnalysis({ nComponents: 2 }),
    ...
    new MiniBatchNMF({ nComponents: 2, batchSize: 4 }),
    ...
    new MultiOutputClassifier(() => new SVC({ kernel: "linear" })),
    new MultiOutputRegressor(() => new Ridge({ alpha: 1 })),
    new ClassifierChain(() => new SVC({ kernel: "linear" }), { order: [0, 1], randomState: 7 }),
    new RegressorChain(() => new Ridge({ alpha: 1 }), { order: [0, 1], randomState: 7 }),
    new Ridge({ alpha: 1 }),
